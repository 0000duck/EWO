        ...
        internal static void Holla(int file_index)
        {
            ...
            if (((swModel != null)))
            {
                ...
                while (true)
                {
                    ...
                    {
                        FromFaces = new Face2[FromFaceObj.Length];
                        ToFaces = new Face2[ToFacesObj.Length];
                        for (int i = 0; i < FromFaceObj.Length; i++)
                        {
                            FromFaces[i] = FromFaceObj[i] as Face2;
                        }
                        ...
                        EWO ewo = new EWO(FromFaces, ToFaces, swCosmeticWeldBeadFeatureData, swModel, swModelDocExt, swApp);
                    ...
                    {
                        EWO ewo = new EWO(swCosmeticWeldBeadFeatureData, swModel, swModelDocExt, swApp);
                        Matrix<Double> joiningNormals = ewo.JoiningNormals;
                        Matrix<Double> baseNormals = ewo.BaseNormals;
                        int ewoindex = 0;
                ...
                //                    XmlElement EWOElement = XMLFile.CreateElement("EWO");
                //                    WeldmentElement.AppendChild(EWOElement);
                //                    EWOElement.SetAttribute("No", Convert.ToString(ewoindexx));
                //                    EWOElement.SetAttribute("StartPose", StartPose);
                //                    EWOElement.SetAttribute("EndPose", EndPose);
